...
int main()
{
	...
	long int Nx = 201, mstep = 1000, Nx1=25601;
	...
	for (inc = 0; inc < nc; inc++) {
		...
		for (kk = 1; kk <= mstep; kk++) {
			...
			for (i = 0; i < Nx; i++) {
				...
				Bx = ue*sk[i]*sk[i]/(t2A);
				// Bx2 = ue*sk2[i]*sk2[i]/(t2B);
				GA_xx=ue*ue*( (sk[i]-0.5)*(0.5-1.0/(6.0*t2A)*(1.0-2.0/t2A))/t2A 
									+ 0.5*atan(2.0*sk[i]-1.0) );
				// GB_xx=ue*ue*( (sk2[i]-0.5)*(0.5-1.0/(6.0*t2B)*(1.0-2.0/t2B))/t2B 
				// 		+ 0.5*atan(2.0*sk2[i]-1.0) );
			...
			// B[1][Nx-1]=B[1][Nx-2];
			// B[2][Nx-1]=B[2][Nx-2];
			// A[1][Nx-1]= 1.0 - A[0][Nx-1]-A[2][Nx-1];
			// A[0][0]=A[0][1];
			// A[1][0]=A[1][1];
			// A[2][0]=A[2][1];
			//**********************************Macro*************************************************
