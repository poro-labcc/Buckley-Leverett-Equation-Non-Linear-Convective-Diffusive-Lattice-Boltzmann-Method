...
int main()
{
	...
	long int Nx = 201, mstep = 1000, Nx1=3201;
	...
	for (inc = 0; inc < nc; inc++) {
		...
		for (i = 0; i < Nx; i++) {
			sk[i] = 0.0;
			sk2[i] = (1.0-sk[i]);
			if ( float(i)/float(Nx)> (1.0-1.0/sqrt(2.0)) ){
				sk[i] = 1.0;
				sk2[i] = (1.0-sk[i]);
			}
			Bx=ue*sk[i]*sk[i]/(sk[i]*sk[i]+(1.0-sk[i])*(1.0-sk[i])) * (1.0-5.0*(1.0-sk[i])*(1.0-sk[i]));
			Bx2 = ue*sk2[i]*sk2[i]/(sk2[i]*sk2[i]+(1.0-sk2[i])*(1.0-sk2[i])) * (1.0-5.0*(1.0-sk2[i])*(1.0-sk2[i]));
			for (k = 0; k < 3; k++) {
		...
		for (kk = 1; kk <= mstep; kk++) {
			//--------------------------------------Colisao---------------------------------------------------
			for (i = 0; i < Nx; i++) {
				t2A=sk[i]*sk[i]+(1.0-sk[i])*(1.0-sk[i]);
				t2B=sk2[i]*sk2[i]+(1.0-sk2[i])*(1.0-sk2[i]);
				Bx = ue*sk[i]*sk[i]/(t2A) * (1.0-5.0*(1.0-sk[i])*(1.0-sk[i]));
				Bx2 = ue*sk2[i]*sk2[i]/(t2B) * (1.0-5.0*(1.0-sk2[i])*(1.0-sk2[i]));
				GA_xx=ue*ue*( 25.0*sk[i]*(sk[i]*sk[i]/3.0-sk[i]/2.0+1.0/4.0) + 1.0/(24.0*t2A*t2A*t2A)*(-42.0*sk[i]+ 1.0) + 
						1.0/(16.0*t2A*t2A)*(14.0*sk[i] + 13.0) + 1.0/(16.0*t2A)*(258.0*sk[i] - 89.0) +
            			5.0/2.0*log(t2A) - 71.0/8.0* atan(2.0*sk[i]-1.0) );
				GB_xx=ue*ue*( 25.0*sk2[i]*(sk2[i]*sk2[i]/3.0-sk2[i]/2.0+1.0/4.0) + 1.0/(24.0*t2B*t2B*t2B)*(-42.0*sk2[i]+ 1.0) + 
						1.0/(16.0*t2B*t2B)*(14.0*sk2[i] + 13.0) + 1.0/(16.0*t2B)*(258.0*sk2[i] - 89.0) +
            			5.0/2.0*log(t2B) - 71.0/8.0* atan(2.0*sk2[i]-1.0) );
				PsiA=-4.0*(sk[i]*sk[i]*sk[i]/3.0-sk[i]*sk[i]/2.0);
				PsiB=-4.0*(sk2[i]*sk2[i]*sk2[i]/3.0-sk2[i]*sk2[i]/2.0);
				LaA=GA_xx+cs*cs*PsiA;
				LaB=GB_xx+cs*cs*PsiB;
				PiA_x=0.0;
				PiB_x=0.0;
				for (k = 0; k < 3; k++) {
					Aeq = w[k]*(sk[i] + Bx*3.0*cx[k] +4.5*(LaA-sk[i]/3.0)*(cx[k]*cx[k]-1.0/3.0) );
					Beq = w[k]*(sk2[i] + Bx2*3.0*cx[k]+4.5*(LaA-sk2[i]/3.0)*(cx[k]*cx[k]-1.0/3.0));
					PiA_x = PiA_x + (A[k][i] - Aeq)*cx[k];
					PiB_x = PiB_x + (B[k][i] - Beq)*cx[k];
				}
				for (k = 0; k < 3; k++) {
					Aeq = w[k]*(sk[i] + Bx*3.0*cx[k] +4.5*(LaA-sk[i]/3.0)*(cx[k]*cx[k]-1.0/3.0) );
					Beq = w[k]*(sk2[i] + Bx2*3.0*cx[k]+4.5*(LaA-sk2[i]/3.0)*(cx[k]*cx[k]-1.0/3.0));
					Aneq=w[k]*(cx[k]*PiA_x*3.0);
					Bneq=w[k]*(cx[k]*PiB_x*3.0);
					Ar[k][i] = Aeq + (1.0-1.0/tau)*Aneq;
					Br[k][i] = Beq + (1.0-1.0/tau)*Bneq;
				}
			}
			//---------------------------------------Streaming-FHH-BC-------------------------------------------------
			for (i = 0; i < Nx; i++) {
			...
			// B[1][0]=B[1][1];
			// B[2][0]=B[2][1];
			B[1][0]= 1.0 - B[0][0]-B[2][0];
			B[0][Nx-1]=B[0][Nx-2];
			B[1][Nx-1]=B[1][Nx-2];
			B[2][Nx-1]=B[2][Nx-2];
			A[2][Nx-1]= 1.0 - A[0][Nx-1]-A[1][Nx-1];
			A[0][0]=A[0][1];
			A[1][0]=A[1][1];
			A[2][0]=A[2][1];
			//**********************************Macro*************************************************
